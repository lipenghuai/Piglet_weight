...
try:
    ...
    _YOLO_BLOCK_IMPORT_ERROR = None
except Exception as exc:  # pragma: no cover - only used outside Ultralytics.
    _YOLO_BLOCK_IMPORT_ERROR = exc

    class _MissingYOLOBlock(nn.Module):
        def __init__(self, *args, **kwargs):
            raise ImportError(
                "ABlockMoE and A2C2fMoE require ultralytics.nn.modules.block. "
                "Copy this package into a YOLO/Ultralytics tree, or use the "
                "standalone MoE layers such as ModularRouterExpertMoE."
            ) from _YOLO_BLOCK_IMPORT_ERROR

    ABlock = A2C2f = C3k = _MissingYOLOBlock
